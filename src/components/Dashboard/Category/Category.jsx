...
import { useGetAllCategory } from "../../Api/Api";
...
export function CreateCategory() {
  ...
  async function onSubmit(values) {
    ...
    try {
      if (!editSlug) {
        ...
        await api.put(`/category/update-category/${editSlug}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setEditSlug(null);
      }
  ...
  const handleDelete = async (slug) => {
    const token = JSON.parse(localStorage.getItem("accessToken"));
    try {
      await api.delete(`/category/delete-category/${slug}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      refetch();
    } catch (err) {
    ...
    <div className="p-6 space-y-10">
      ...
      <Form {...form}>
        ...
        >
          ...
          <Button type="submit">
            {editSlug ? "Update Category" : "Create Category"}
          </Button>
        </form>
      </Form>
